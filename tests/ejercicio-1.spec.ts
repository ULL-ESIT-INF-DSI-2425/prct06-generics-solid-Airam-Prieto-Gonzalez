...
import { Series, Films, Documentals } from "../src/ejercicio-1/Collection_Classes"
...
describe("ejercicio-1", () => {
        test("SerieCollection", () => {
                ...
                expect(SerieCollection.SearchByYear(2023)).toBe(' STREAMS PUBLISHED IN 2023 : The Last Of Us, Just Testing, ')
                expect(SerieCollection.SearchBySeasons(2)).toBe(' STREAMS OF 2 SEASONS : Squid Game, Just Testing, ')
        })
        test("FilmCollection", () => {
                let PulpFiction = new Film("Pulp Fiction", 1994, 120, "Tarantino", "Adventure")
                ...
                expect(FilmCollection.SearchByYear(1994)).toBe(' STREAMS PUBLISHED IN 1994 : Pulp Fiction, ')
                expect(FilmCollection.SearchByGender('Adventure')).toBe(' STREAMS OF Adventure: Pulp Fiction, ')
        })
})