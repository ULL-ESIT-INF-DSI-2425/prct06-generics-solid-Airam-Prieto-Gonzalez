...
describe('Music Archive Tests', () => {
        // Test instances
        const blinding_lights = new Song("Blinding Lights", 202, "Pop", true, 2500000000);
        const save_your_tears = new Song("Save Your Tears", 215, "Pop", true, 1800000000);
        const after_hours = new Song("After Hours", 361, "Pop", false, 800000000);
        const A_after_hours = new Single("After Hours", 2020, blinding_lights, save_your_tears, after_hours);
        ...
        const A_dawn_fm = new Disc("Dawn FM", 2022, sacrifice, out_of_time, less_than_zero);

        const TW_discography = new Discography<Single|Disc>("The Weeknd Albums", A_after_hours, A_dawn_fm);
        ...
        test('should find the singer by name', () => {
                expect(Library.SearchSinger('The Weeknd')).toStrictEqual([["The Weeknd", 23000000000,"   -After Hours   -Dawn FM",]]);
                expect(Library.SearchSinger('Unknown')).toBeUndefined();
        });

        test('should find the disc by name', () => {
                expect(Library.SearchDisc('After Hours')).toStrictEqual([["After Hours", 'The Weeknd', 2020, "   -Blinding Lights   -Save Your Tears   -After Hours",]]);
                expect(Library.SearchDisc('Unknown')).toBeUndefined();
        });

        test('should find the song by name', () => {
                expect(Library.SearchSong('Blinding Lights')).toStrictEqual([["Blinding Lights", 'The Weeknd', "Pop", 202, 2500000000]]);
                expect(Library.SearchSong('Unknown')).toBeUndefined();
        });

        test('should calculate total views of a disc', () => {
                expect(Library.TotalViews('After Hours')).toBe(2500000000 + 1800000000 + 800000000);
        });
